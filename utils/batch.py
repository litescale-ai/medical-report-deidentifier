...
async def discover_names(text, *, model, base_url):
        ...
        try:
            ...
            for part_number, part in enumerate(parts, 1):
                ...
                if chunk_key not in state['chunks']:
                    ...
                    state['chunks'][chunk_key] = await discover_names(part, model=model, base_url=base_url)
    ...
    for source, relative, prepared, sections, fingerprint in records:
        ...
        try:
            ...
            if temporary and os.path.exists(temporary):
                os.unlink(temporary)
    return {'completed': completed, 'failed': failures, 'reused': reused,
